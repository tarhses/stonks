...
const { AUCTION_STATE, OFFER_STATE, RECREATE_ROOM, SET_TIMER } = require('../common/signals')

module.exports = class RecoveryRoom {
  roomId
  players
  animals
  status
  ...
  recreate (io) {
    ...
    if (this.status && this.status.type === AUCTION_STATE) {
      room.emit(SET_TIMER, room.state.timeout)
    }

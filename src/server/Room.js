...
function deserializeStatus(room, data) {
	...
	switch (data.type) {
		case TURN_STATE:
			return Turn.deserialize(room, data)
		case AUCTION_STATE:
			return Auction.deserialize(room, data)
		case AUCTION_END_STATE:
			return AuctionEnd.deserialize(room, data)
		case OFFER_STATE:
			return Offer.deserialize(room, data)
		case END_STATE:
			return End.deserialize(room, data)
	}
}
...
export default class Room {
	...
	static deserialize(io, { players, animals, status, roomId }) {
		...
		room.players = players.map((player, id) =>
			Player.deserialize(room, id, player),
		)

...
export default function Capital({ capital, offer, onSelected }) {
	...
	return (
		<div>
			...
			{offer ? (
				<p>
					<b>Selected:</b> {selected.reduce((acc, value) => acc + value, 0)}$ (
					{selected.length}/{capital.length}).
				</p>
			) : (
				<p>
					<b>Total:</b> {capital.reduce((sum, value) => sum + value, 0)}$.
				</p>
			)}

			{offer && (
				<button onClick={() => onSelected(selected)}>Make offer</button>
			)}
		</div>
	)
}

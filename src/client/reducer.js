		...
		case SELL_ANIMAL: {
			return state.map((player, playerId) => {
				if (playerId === action.sellerId || playerId === action.buyerId) {
					...
					const count =
						action.count === 0 && playerId === action.buyerId ? 1 : action.count
					return {
						...player,
...
function capital(state, action) {
	switch (action.type) {
		...
		case PAY_CAPITAL:
			return action.capital.reduce(
				(acc, value) => {
					acc.splice(
						acc.findIndex((item) => item === value),
						1,
					)
					return acc
				},
...
function animals(state, action) {
	switch (action.type) {
		case START_AUCTION:
			return state.map((count, id) =>
				id === action.animalId ? count - 1 : count,
			)
	...
	switch (action.type) {
		...
		case RESTART_AUCTION:
			return {
				...
				animalId:
					action.type === START_AUCTION ? action.animalId : state.animalId,
				amount: 0,
...
export default (state, action) => {
	if (action.type === JOIN_ROOM) {
		...
		return {
			...
			roomId: state.roomId,
			selfId:
				action.type === REMOVE_PLAYER && state.selfId > action.playerId
					? state.selfId - 1
					: state.selfId,
		}
	}
